...
int Spider_Analytical_HTML(url_t node,container_t * E_ct,container_t * P_ct){
    ...
    if((map_ptr = mmap(NULL,filesize,PROT_READ,MAP_PRIVATE,fd,0))==MAP_FAILED){
        perror("Analytical HTML >> mmap Failedmka");
        exit(0);
    }
    ...
    Spider_Save_Result(h1,des,node.origin_url);
    printf("Spider Analytical HTML,Result Title=[%s] ,Description=[%s]\n",h1,des);
